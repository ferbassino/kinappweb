export const courses = [
  {
    id: "1",
    name: "jump",
    title: "curso jump kinapp",
    quantity: 1,
    price: 40000,
    buttonTitle: "inscribite ahora",
    descriptionTitle: "Salto vertical",
    subTitle:
      "Curso de análisis del salto vertical con unidades inerciales y registro por video",
    description:
      "En este curso de analizan diferentes tipos de saltos verticales como el drop-jump, counter-movement-jump, squat-jump utilizando unidades inerciales y software de análisis por video",
    img: "/jump.png",
    video: "/preview.mp4",
    program: [
      {
        title:
          "MODULO DE INTRODUCCION AL ENTORNO KINAPP Y PRESENTACION DEL CURSO",
        subTitle: "INTRODUCCIÓN AL SALTO",
        content: [
          "Presentación de equipo y del entorno kinApp",
          "Presentación del programa, plataforma y objetivos del curso",
          "Tipos de salto y baterías de evaluación",
          "Relación del salto en el deporte y el rendimiento",
          "Métodos de evaluación (plataforma de fuerza, alfombra de contacto, IMU)",
          "Sistema de referencia e introducción a la Dinámica",
        ],
      },
      {
        title: "BIOMECANICA Y EVALUACION DEL SJ Y CMJ",
        subTitle: "SQUAT JUMP Y COUNTERMOVEMENT JUMP",
        content: [
          "Análisis cinemático segmentario y puntual (CM) del SJ",
          "Análisis dinámico",
          "Variables de interés biomecánico",
          "Análisis cinemático segmentario y puntual (CM) del SJ",
          "Análisis dinámico",
          "Variables de interés biomecánico",
          "Traslación al análisis con video y con IMU ",
        ],
      },
      {
        title: "BIOMECANICA Y EVALUACION DEL DROP JUMP Y STIFFNESS”",
        subTitle: "ABSORCIONDE CARGAS, ENERGIA ELASTICA",
        content: [
          "Física de la colisión",
          "Dinámica y análisis del aterrizaje",
          "Energía potencial elástica y Stiffness",
          "Análisis cinemático y dinámico mediante video y aplicación móvil para el Drop Jump y el Stiffness",
        ],
      },
    ],
  },

  // {
  //   id: "2",
  //   name: "video",
  //   title: "Video",
  //   quantity: 1,
  //   price: 40000,
  //   buttonTitle: "inscribite ahora",
  //   descriptionTitle: "Videoanálisis",
  //   subTitle:
  //     "Curso de análisis del salto vertical con unidades inerciales y registro por video",
  //   description:
  //     "En este curso de analizan diferentes tipos de saltos verticales como el drop-jump, counter-movement-jump, squat-jump utilizando unidades inerciales y software de análisis por video",
  //   img: "storage.jpg",
  //   program: [
  //     {
  //       title:
  //         "MODULO DE INTRODUCCION AL ENTORNO KINAPP Y PRESENTACION DEL CURSO",
  //       subTitle: "INTRODUCCIÓN AL SALTO",
  //       content: [
  //         "Presentación de equipo y del entorno kinApp",
  //         "Presentación del programa, plataforma y objetivos del curso",
  //         "Tipos de salto y baterías de evaluación",
  //         "Relación del salto en el deporte y el rendimiento",
  //         "Métodos de evaluación (plataforma de fuerza, alfombra de contacto, IMU)",
  //         "Sistema de referencia e introducción a la Dinámica",
  //       ],
  //     },
  //     {
  //       title: "BIOMECANICA Y EVALUACION DEL SJ Y CMJ",
  //       subTitle: "SQUAT JUMP Y COUNTERMOVEMENT JUMP",
  //       content: [
  //         "Análisis cinemático segmentario y puntual (CM) del SJ",
  //         "Análisis dinámico",
  //         "Variables de interés biomecánico",
  //         "Análisis cinemático segmentario y puntual (CM) del SJ",
  //         "Análisis dinámico",
  //         "Variables de interés biomecánico",
  //         "Traslación al análisis con video y con IMU ",
  //       ],
  //     },
  //     {
  //       title: "BIOMECANICA Y EVALUACION DEL DROP JUMP Y STIFFNESS”",
  //       subTitle: "ABSORCIONDE CARGAS, ENERGIA ELASTICA",
  //       content: [
  //         "Física de la colisión",
  //         "Dinámica y análisis del aterrizaje",
  //         "Energía potencial elástica y Stiffness",
  //         "Análisis cinemático y dinámico mediante video y aplicación móvil para el Drop Jump y el Stiffness",
  //       ],
  //     },
  //   ],
  // },
  // {
  //   id: "3",
  //   name: "basico",
  //   title: "biomecanica basico",
  //   quantity: 1,
  //   price: 40000,
  //   buttonTitle: "inscribite ahora",
  //   descriptionTitle: "Biomecánica inicial",
  //   subTitle:
  //     "Curso de análisis del salto vertical con unidades inerciales y registro por video",
  //   description:
  //     "En este curso de analizan diferentes tipos de saltos verticales como el drop-jump, counter-movement-jump, squat-jump utilizando unidades inerciales y software de análisis por video",
  //   img: "time.jpg",
  //   program: [
  //     {
  //       title:
  //         "MODULO DE INTRODUCCION AL ENTORNO KINAPP Y PRESENTACION DEL CURSO",
  //       subTitle: "INTRODUCCIÓN AL SALTO",
  //       content: [
  //         "Presentación de equipo y del entorno kinApp",
  //         "Presentación del programa, plataforma y objetivos del curso",
  //         "Tipos de salto y baterías de evaluación",
  //         "Relación del salto en el deporte y el rendimiento",
  //         "Métodos de evaluación (plataforma de fuerza, alfombra de contacto, IMU)",
  //         "Sistema de referencia e introducción a la Dinámica",
  //       ],
  //     },
  //     {
  //       title: "BIOMECANICA Y EVALUACION DEL SJ Y CMJ",
  //       subTitle: "SQUAT JUMP Y COUNTERMOVEMENT JUMP",
  //       content: [
  //         "Análisis cinemático segmentario y puntual (CM) del SJ",
  //         "Análisis dinámico",
  //         "Variables de interés biomecánico",
  //         "Análisis cinemático segmentario y puntual (CM) del SJ",
  //         "Análisis dinámico",
  //         "Variables de interés biomecánico",
  //         "Traslación al análisis con video y con IMU ",
  //       ],
  //     },
  //     {
  //       title: "BIOMECANICA Y EVALUACION DEL DROP JUMP Y STIFFNESS”",
  //       subTitle: "ABSORCIONDE CARGAS, ENERGIA ELASTICA",
  //       content: [
  //         "Física de la colisión",
  //         "Dinámica y análisis del aterrizaje",
  //         "Energía potencial elástica y Stiffness",
  //         "Análisis cinemático y dinámico mediante video y aplicación móvil para el Drop Jump y el Stiffness",
  //       ],
  //     },
  //   ],
  // },
  // {
  //   id: "4",
  //   name: "intermedio",
  //   title: "biomecanica basico",
  //   quantity: 1,
  //   price: 40000,
  //   buttonTitle: "inscribite ahora",
  //   descriptionTitle: "Biomecánica inicial",
  //   subTitle:
  //     "Curso de análisis del salto vertical con unidades inerciales y registro por video",
  //   description:
  //     "En este curso de analizan diferentes tipos de saltos verticales como el drop-jump, counter-movement-jump, squat-jump utilizando unidades inerciales y software de análisis por video",
  //   img: "time.jpg",
  //   program: [
  //     {
  //       title:
  //         "MODULO DE INTRODUCCION AL ENTORNO KINAPP Y PRESENTACION DEL CURSO",
  //       subTitle: "INTRODUCCIÓN AL SALTO",
  //       content: [
  //         "Presentación de equipo y del entorno kinApp",
  //         "Presentación del programa, plataforma y objetivos del curso",
  //         "Tipos de salto y baterías de evaluación",
  //         "Relación del salto en el deporte y el rendimiento",
  //         "Métodos de evaluación (plataforma de fuerza, alfombra de contacto, IMU)",
  //         "Sistema de referencia e introducción a la Dinámica",
  //       ],
  //     },
  //     {
  //       title: "BIOMECANICA Y EVALUACION DEL SJ Y CMJ",
  //       subTitle: "SQUAT JUMP Y COUNTERMOVEMENT JUMP",
  //       content: [
  //         "Análisis cinemático segmentario y puntual (CM) del SJ",
  //         "Análisis dinámico",
  //         "Variables de interés biomecánico",
  //         "Análisis cinemático segmentario y puntual (CM) del SJ",
  //         "Análisis dinámico",
  //         "Variables de interés biomecánico",
  //         "Traslación al análisis con video y con IMU ",
  //       ],
  //     },
  //     {
  //       title: "BIOMECANICA Y EVALUACION DEL DROP JUMP Y STIFFNESS”",
  //       subTitle: "ABSORCIONDE CARGAS, ENERGIA ELASTICA",
  //       content: [
  //         "Física de la colisión",
  //         "Dinámica y análisis del aterrizaje",
  //         "Energía potencial elástica y Stiffness",
  //         "Análisis cinemático y dinámico mediante video y aplicación móvil para el Drop Jump y el Stiffness",
  //       ],
  //     },
  //   ],
  // },
];
